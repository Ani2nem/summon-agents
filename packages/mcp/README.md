...
2. **Dispatch** — one agent per task, each in its own `git worktree`, running **headless**. No mid-task prompts. That's the whole point — you leave. (Worktrees fork from your **last commit** - so uncommitted changes in your working tree aren't seen by the agents. summon-agents warns you up front if you have any; commit or stash them first if the agents should build on them.)
